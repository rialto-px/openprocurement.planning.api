...
class PlanResourceTest(BaseWebTest):
    ...
    def test_create_plan_invalid(self):
        ...
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertIn({u'description': {u'procurementMethodType': [u"Value must be one of ('belowThreshold', 'aboveThresholdUA', 'aboveThresholdEU', 'aboveThresholdUA.defense', 'aboveThresholdTS')."]}, u'location': u'body', u'name': u'tender'},
                     response.json['errors'])
    ...
    def test_create_plan_generated(self):
        ...
        self.assertEqual(set(plan), set([
            ...
            u'classification', u'items', u'procuringEntity', u'owner'
        ]))
